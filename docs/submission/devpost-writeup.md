...
Every surface is hosted: the gateway at `0g-gateway.equiflow.xyz` and the dashboard at `agentgate.equiflow.xyz`, both reading that deployment. Mind the `0g-` prefix: without it is an older Casper one.
